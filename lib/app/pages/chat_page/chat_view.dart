...
class _ChatViewState extends State<ChatView> with TickerProviderStateMixin {
  ...
  void sendMsg() {
    if (messageEditingController.text.isNotEmpty) {
      setState(() {
        messages.add(ChatModel(text: messageEditingController.text, date: DateTime.now(), isSentByMe: true));
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: isMobileOrTablet ? _buildMobileView(sizeInfo) : _buildDesktopView(sizeInfo),
    );
  }
  ...
  Widget _buildChatScreen() {
    ...
    if (selectedUser == null || !filteredUsers.any((user) => user.influencerName == selectedUser)) {
    ...
    return ShadowContainer(
      ...
      showHeader: false,
      child: ChatScreen(receiver: filteredUsers.firstWhere((user) => user.influencerName == selectedUser)),
    );
  }
  ...
  bool _isMobileOrTablet(BuildContext context) {
    return rf.ResponsiveValue<bool>(
      context,
      conditionalValues: const [rf.Condition.between(start: 0, end: 1023, value: true)],
      defaultValue: false,
    ).value;
  }
    ...
    return rf.ResponsiveValue<_SizeInfo>(
      ...
      conditionalValues: [
        const rf.Condition.between(
            start: 0, end: 599, value: _SizeInfo(alertFontSize: 12, padding: EdgeInsets.all(16), innerSpacing: 16)),
        const rf.Condition.between(
            start: 600, end: 1023, value: _SizeInfo(alertFontSize: 16, padding: EdgeInsets.all(20), innerSpacing: 20)),
        const rf.Condition.between(
            start: 1024, end: 9999, value: _SizeInfo(alertFontSize: 18, padding: EdgeInsets.all(24), innerSpacing: 24)),
      ],
